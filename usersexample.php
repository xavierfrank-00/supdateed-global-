...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['action']) && $_GET['action'] === 'register') {
    ...
    $sql = "SELECT * FROM usersexample WHERE email = ?";
    ...
    $sql = "INSERT INTO usersexample (name, email, password) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($sql);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['action']) && $_GET['action'] === 'login') {
    ...
    if ($user && password_verify($password, $user['password'])) {
        $token = generateToken();
        $sql = "UPDATE usersexample SET token = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
...
function checkToken($conn, $token) {
    $sql = "SELECT * FROM usersexample WHERE token = ?";
    $stmt = $conn->prepare($sql);
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT' && isset($_GET['action']) && $_GET['action'] === 'update_user') {
    // No need to fetch token from body; it's in the URL now
    $token = $_GET['token'];
    $data = getRequestBody();
    ...
    if ($user) {
        $sql = "UPDATE usersexample SET name = ?, email = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
