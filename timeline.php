...
try {
    ...
    $stmt = $pdo->prepare("CALL PR_USER_TIMELINE(:date, :enddate, :empid, :empname, :team, :role, :department, :project, :userid)");
    ...
    $stmt->bindParam(':userid', $userid); // Bind the new userid parameter
...
// Set enddate to the same value as date if not provided
$enddate = isset($_POST['enddate']) && !empty($_POST['enddate']) ? $_POST['enddate'] : $date;
    ...
    $userid = isset($parsedData['userid']) ? $parsedData['userid'] : 'ALL'; // Get the userid from the input
    $stmt->execute();
